...
class CardPile {
	...
	public:
		...
		// merge sort algorithm using the less function above
		// result is ascending
		CardPile mergeSort(CardPile c) {
			int size = c.cards.size();
			if (size <= 1) return c;
			CardPile left, right, result;
			int middle = size/2;
			for (int i = 0; i < middle; i++)
				left.cards.push_back(c.cards[i]);
			for (int i = middle; i < size; i++)
				right.cards.push_back(c.cards[i]);
			left = mergeSort(left);
		...
		// used in the above merge sort algorithm
		CardPile merge(CardPile left, CardPile right) {
			CardPile result;
			while (left.cards.size() > 0 && right.cards.size() > 0) {
				if (left.cards.front().less(right.cards.front())) {
					result.cards.insert(result.cards.end(),
						left.cards.begin(), ++left.cards.begin());
					left.cards.erase(left.cards.begin());
				}
				else {
					result.cards.insert(result.cards.end(),
						right.cards.begin(), ++right.cards.begin());
					right.cards.erase(right.cards.begin());
				}
			}
			if (left.cards.size() > 0) {
				result.cards.insert(result.cards.end(),
					left.cards.begin(), left.cards.end());
			}
			else {
				result.cards.insert(result.cards.end(),
					right.cards.begin(), right.cards.end());
			}
			return result;
		}
